      ...
      included do
        ...
        def set_confessions
          if params[:q]
            ...
            combined_results = (ransack_results + user_results + action_text_results).uniq.sort_by(&:created_at).reverse
            @confessions = Kaminari.paginate_array(combined_results).page(params[:page]).per(3)
          else
            @confessions = Confession.includes(:rich_text_content, :user).order(created_at: :desc).page(params[:page]).per(3)
          end
